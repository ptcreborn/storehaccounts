            ...
            if (data.hasOwnProperty(footerComment.comment_footer_url)) {
                ...
                for (i = 0; i < data[footerComment.comment_footer_url].comments.length; i++) {
                    ...
                    for (j = 0; j < temp_data.replies.length; j++) {
                        ...
                        let temp_data = await JBLOBFunctions.getBlobRecordSync('https://jsonblob.com/api/jsonBlob/' + reply_comment_id, null);
                        ...
                        reply_build += "<div id=" + reply_comment_id + " class='comment-replied'><img class='user-profile' src='" + temp_data.userimg + "'/><a target='_blank' href='" + temp_data.userid + "'>" + temp_data.username + "</a><span>replied " + moment(parseInt(temp_data.date)).fromNow() + "</span><p>" + temp_data.content.replaceAll('\n', '<br/>') + "</p>" + div_imgs.outerHTML + "</div>";
                    }
